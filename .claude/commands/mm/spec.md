...
```
/mm:spec ["short description"] [--stage shape|write|tasks [spec-folder]] [--phase [n]] [--auto]
```
...
**When delegating to any subagent (spec-shaper, spec-writer, task-list-creator), pass the ABSOLUTE
...
modes section (the canonical definition; do not restate it here).
...
`mm_create({entity: "spec", project_path, name, description, source, depends_on?, idea_folder?, issue_path?})`
...
treat the shaper's `requirements.md` as a starting point: spec-writer and task-list-creator re-verify
...
4a. Delegate to **task-list-creator** with `spec.md`, `requirements.md`, refreshed `prior-work.md`,
...
See task-list-creator.md's scope-gate probe and Output contract sections for the canonical detail.
...
4c. On REVISE, feed critique back to task-list-creator. Max 2 revisions; then proceed with the best version and note remaining concerns.
...
- **`--stage tasks`** — step 4's full reflection loop (4a–4d) for an existing spec: needs
  ...
  Run task-list-creator → critique → register via ONE batch `mm_create({entity: "task", titles})`
...
3. **Create each spec** through THIS command's own pipeline, abbreviated per feature:
   ...
   source-docs AND the current codebase → **task-list-creator** produces the task list →
