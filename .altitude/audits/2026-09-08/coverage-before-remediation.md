...
- **`al-text-block` — RESOLVED 2026-09-06.** The "Text Passage" set had been deleted outright (the manifest pin was a detached ghost), so the rename was settled in the code's favour: the manifest maps `Text Block`, and the set was regenerated on 🛠 Text Block. It generates as a SINGLE variant — its only prop (`maxWidth`) is `figma.omit` and its only state (`hover`) carries no styling delta — so the old set's `Width` axis is backed by no contract fact. It does NOT pass `check-parity` (figma 390x18 vs browser 440x24), so it is not stamped synced.
...
- **"Input" mapping** — RESOLVED 2026-09-06: a digest refresh repopulated it (`nodeId: 3544:48650`). All 34 pinned ids are live (`pnpm run parity:pins`).
- **`al-combobox` — REGENERATED 2026-09-06.** The old set had been dissolved to a lone `COMPONENT` ("Combobox/Default"); it is a real 2-variant set again, pinned at `3694:11119`. Both variants render the SAME anatomy because `anatomySource` captured exactly one case (`Label=shown`) — named in the run's degradations — and it does NOT pass `check-parity` (figma 229x140 vs browser 229x64 / 229x92), so it is not stamped synced. The owner's original "Combobox" frame, with the old lone component, is still on the page.
