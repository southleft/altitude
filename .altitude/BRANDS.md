...
| Body-md size / leading | 16 / 24 | **14 / 20** | **18 / 32** | 16 / **20** |
| Radius `@` / `lg` | 4 / 8 | **2 / 4** | **12 / 24** | **0 / 0** |
| Border `@` / `md` | 1 / 2 | 1 / 2 | 1 / 2 | **2 / 4** |
| Shadow `@` / `md` | 8 / 16 | **2 / 4** | **16 / 32** | **hard-2 / hard-4** |
| Shadow tint | neutral black | neutral black | **warm brown** | **brand red, zero blur** |
| Space `xxs` / `xs` / `@` | 4 / 8 / 16 | **2 / 4 / 8** | **8 / 12 / 24** | 4 / 8 / 16 |
| Non-colour axes varied | 0 (by design) | typography, radius, shadow, spacing | typography, radius, shadow (+tint), spacing | typography, radius, border-width, shadow |

Each expressive brand varies four non-colour axes — one more than R5's floor of
three. northright and southleft deliberately diverge on *different* axes:
northright takes density (spacing + type size) and leaves border weight alone;
southleft takes weight (border width + zero radius + hard shadow) and keeps
altitude's spacing ladder, so the two do not collapse into each other.
That is why the altitude-vs-southleft pair is the weakest at 3 of 5 marker
properties while every other pair hits 5 of 5.
...
`build-brand-compare.mjs` additionally fails if the four brands do not resolve
to four *distinct* computed `font` shorthands on the same rendered element —
a CSS-variable diff proves the token moved, only a computed style proves it
reached the component. Measured 2026-07-28 on `al-button`'s label, read inside
the shadow root:

| Brand | computed `font` |
|---|---|
| altitude | `600 16px/24px "IBM Plex Sans", sans-serif` |
| northright | `600 14px/20px "IBM Plex Sans", sans-serif` |
| odyssey | `600 18px/32px Georgia, Cambria, serif` |
