...
Re-measured 2026-08-30 from the emitted bundles
(`dist-v5/css/brand/tokens-<brand>-dark.css`) after the v2 restyle — do not
hand-edit these; read them back out of the build.

| | altitude | southleft |
|---|---|---|
| Archetype | general-purpose reference | high-contrast utilitarian |
| Accent ramp | blue | red |
| Type family | Public Sans | **IBM Plex Sans** |
| Body-md size / leading | 16 / 24 | 16 / **20** |
| Radius `@` / `lg` | 6 / 12 | **2 / 4** |
| Border `@` / `md` | 1 / 2 | 1 / **4** |
| Shadow `@` / `md` | raised / overlay | **none / 4** |
...
**Superseded 2026-08-30 by the v2 restyle.** `styles/main.scss:11` now fetches
**Public Sans** (the v2 UI face) and **IBM Plex Mono** (the metadata layer:
table column headers, token names, timestamps).

IBM Plex Sans is no longer in the base bundle. southleft still renders it — its
`typography-primitives.json` pins `{font-family.plex}`, a tier-1 primitive added
precisely so the brand could keep its face when `font-family.primary` moved — so
southleft consumers must load it themselves. In this repo
`apps/southleft/src/styles/fonts.css` does that, next to the app's other brand
faces, and `apps/docs/src/styles/docs.css` does the same so southleft-branded
previews resolve correctly. `font-family.secondary` (`Agrandir`) is still
declared-but-not-fetched by the base and is self-hosted by the apps that use it.
...
**Do not create ANY of these files for `altitude`.** See
`tier-2/brand/altitude/README.md`: `tokens-altitude-{light,dark}.css` is
asserted byte-identical to the base bundles, so any file there fails
`test:brands`. Giving altitude character is a re-scope, not a brand edit.

That directory used to hold a `colors.json` restating base values. It was
deleted in the v2 restyle (2026-08-30) because a mode-AGNOSTIC brand file
cannot mirror a role that became mode-DEPENDENT: its single
`content.primary-weak` line pinned the dark on-primary ink into the light
bundle and outranked the mode axis by specificity (0,3,0 vs 0,2,0). Deleting it
does NOT stop the brand bundles emitting — measured 2026-08-30, both still emit
byte-identical to base from the `include` chain alone. Overriding nothing is the
