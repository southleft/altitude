...
| Visual regression | `pnpm run build:fixtures && pnpm test:vrt` — needs `libs/al-web-components/story-fixture/dist/index.json` built first (`build:story-fixture`, part of `build:fixtures`); Playwright serves the fixture itself on `:5178` via `webServer` (see `playwright.config.ts`). **this spec is not meaningful on Windows — remeasured 2026-09-03, all 67 of 67 fail there** while the same commit passes every baseline on the Linux runner. The header used to say 9; that was wrong in the dangerous direction, because someone expecting 9 and seeing 67 is one step from a local `baselines:vrt` that poisons the baselines for everyone. CI is authoritative; to refresh after an intentional visual change use `scripts/adopt-vrt-baselines.mjs` on the failing job's artifact (see `tests/components.vrt.spec.ts` header) |
