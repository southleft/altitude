...
| Stage | Path | Notes |
|---|---|---|
| Editable source | `libs/al-web-components/styles/tokens/**.json` | Legacy `value`/`type` shape. **This is what you edit.** |
| Generated DTCG | `libs/al-web-components/styles/tokens-dtcg/**.json` | `$value`/`$type`. Machine-generated by `scripts/convert-tokens-to-dtcg.js` on every build. Gitignored. **Do not hand-edit.** |
...
The legacy `tokens/` source and the `dist/` mirror survive until T6.2's "zero
legacy components" gate clears — see §"Remaining T6.2 follow-ups".
...
node scripts/ingest-tokens-from-studio.js   # ingest a Figma/Tokens-Studio DTCG export
```

`build:tokens:v5` still exists as a deprecated alias of `build:tokens`
...
`styles/tokens/**` or `styles/tokens-config.v5.mjs` without touching
`.altitude/baselines/`.
...
Gated on the "zero legacy components" sign-off — don't pre-empt:

- `libs/al-web-components/styles/tokens/` is deleted.
- `tokens-dtcg/` becomes the editable source (no longer auto-generated).
...
Already done: `styles/tokens-config.js` deleted (T6.2);
