<!-- GENERATED by scripts/contracts/build-component-docs.mjs from al-testimonial.contract.json + the altitude parity manifest — do not hand-edit; run `pnpm contracts:docs` -->
...
No Figma component set is mapped for `al-testimonial` in the **Altitude Design System** parity manifest (parity status: `missing-in-figma`). Map one in Figma, then run `pnpm run parity:seed`.
...
Source contract: `.altitude/contracts/altitude/al-testimonial.contract.json`.
