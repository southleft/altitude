<!-- GENERATED by scripts/contracts/build-component-docs.mjs from al-icon-bell.contract.json + the altitude parity manifest — do not hand-edit; run `pnpm contracts:docs` -->

# al-icon-bell — Icon Bell
...
No Figma component set is mapped for `al-icon-bell` in the **Altitude Design System** parity manifest (parity status: `missing-in-figma`). Map one in Figma, then run `pnpm run parity:seed`.
...
- Import: `@southleft/al-web-components/components/icon/icons/bell.ts`
- Tag: `al-icon-bell`
...
Source contract: `.altitude/contracts/altitude/al-icon-bell.contract.json`.
