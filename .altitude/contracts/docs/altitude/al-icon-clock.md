<!-- GENERATED by scripts/contracts/build-component-docs.mjs from al-icon-clock.contract.json + the altitude parity manifest — do not hand-edit; run `pnpm contracts:docs` -->

# al-icon-clock — Icon Clock
...
No Figma component set is mapped for `al-icon-clock` in the **Altitude Design System** parity manifest (parity status: `missing-in-figma`). Map one in Figma, then run `pnpm run parity:seed`.
...
- Import: `@southleft/al-web-components/components/icon/icons/clock.ts`
- Tag: `al-icon-clock`
...
Source contract: `.altitude/contracts/altitude/al-icon-clock.contract.json`.
