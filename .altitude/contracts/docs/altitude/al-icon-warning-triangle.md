<!-- GENERATED by scripts/contracts/build-component-docs.mjs from al-icon-warning-triangle.contract.json + the altitude parity manifest — do not hand-edit; run `pnpm contracts:docs` -->
...
No Figma component set is mapped for `al-icon-warning-triangle` in the **Altitude Design System** parity manifest (parity status: `missing-in-figma`). Map one in Figma, then run `pnpm run parity:seed`.
...
- Import: `@southleft/al-web-components/components/icon/icons/warning-triangle.ts`
...
Source contract: `.altitude/contracts/altitude/al-icon-warning-triangle.contract.json`.
