<!-- GENERATED by scripts/contracts/build-component-docs.mjs from al-icon-pin.contract.json + the altitude parity manifest — do not hand-edit; run `pnpm contracts:docs` -->

# al-icon-pin — Icon Pin
...
No Figma component set is mapped for `al-icon-pin` in the **Altitude Design System** parity manifest (parity status: `missing-in-figma`). Map one in Figma, then run `pnpm run parity:seed`.
...
- Import: `@southleft/al-web-components/components/icon/icons/pin.ts`
- Tag: `al-icon-pin`
...
Source contract: `.altitude/contracts/altitude/al-icon-pin.contract.json`.
