<!-- GENERATED by scripts/contracts/build-component-docs.mjs from al-icon-close.contract.json + the altitude parity manifest — do not hand-edit; run `pnpm contracts:docs` -->

# al-icon-close — Icon Close
...
No Figma component set is mapped for `al-icon-close` in the **Altitude Design System** parity manifest (parity status: `missing-in-figma`). Map one in Figma, then run `pnpm run parity:seed`.
...
- Import: `@southleft/al-web-components/components/icon/icons/close.ts`
- Tag: `al-icon-close`
...
Source contract: `.altitude/contracts/altitude/al-icon-close.contract.json`.
