<!-- GENERATED by scripts/contracts/build-component-docs.mjs from al-icon-add.contract.json + the altitude parity manifest — do not hand-edit; run `pnpm contracts:docs` -->

# al-icon-add — Icon Add
...
No Figma component set is mapped for `al-icon-add` in the **Altitude Design System** parity manifest (parity status: `missing-in-figma`). Map one in Figma, then run `pnpm run parity:seed`.
...
- Import: `@southleft/al-web-components/components/icon/icons/add.ts`
- Tag: `al-icon-add`
...
Source contract: `.altitude/contracts/altitude/al-icon-add.contract.json`.
