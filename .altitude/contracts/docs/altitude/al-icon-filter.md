<!-- GENERATED by scripts/contracts/build-component-docs.mjs from al-icon-filter.contract.json + the altitude parity manifest — do not hand-edit; run `pnpm contracts:docs` -->

# al-icon-filter — Icon Filter
...
No Figma component set is mapped for `al-icon-filter` in the **Altitude Design System** parity manifest (parity status: `missing-in-figma`). Map one in Figma, then run `pnpm run parity:seed`.
...
- Import: `@southleft/al-web-components/components/icon/icons/filter.ts`
- Tag: `al-icon-filter`
...
Source contract: `.altitude/contracts/altitude/al-icon-filter.contract.json`.
