<!-- GENERATED by scripts/contracts/build-component-docs.mjs from al-icon-check.contract.json + the altitude parity manifest — do not hand-edit; run `pnpm contracts:docs` -->

# al-icon-check — Icon Check
...
No Figma component set is mapped for `al-icon-check` in the **Altitude Design System** parity manifest (parity status: `missing-in-figma`). Map one in Figma, then run `pnpm run parity:seed`.
...
- Import: `@southleft/al-web-components/components/icon/icons/check.ts`
- Tag: `al-icon-check`
...
Source contract: `.altitude/contracts/altitude/al-icon-check.contract.json`.
