<!-- GENERATED by scripts/contracts/build-component-docs.mjs from al-icon-emoji.contract.json + the altitude parity manifest — do not hand-edit; run `pnpm contracts:docs` -->

# al-icon-emoji — Icon Emoji
...
No Figma component set is mapped for `al-icon-emoji` in the **Altitude Design System** parity manifest (parity status: `missing-in-figma`). Map one in Figma, then run `pnpm run parity:seed`.
...
- Import: `@southleft/al-web-components/components/icon/icons/emoji.ts`
- Tag: `al-icon-emoji`
...
Source contract: `.altitude/contracts/altitude/al-icon-emoji.contract.json`.
