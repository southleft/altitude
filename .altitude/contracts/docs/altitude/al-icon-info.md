<!-- GENERATED by scripts/contracts/build-component-docs.mjs from al-icon-info.contract.json + the altitude parity manifest — do not hand-edit; run `pnpm contracts:docs` -->

# al-icon-info — Icon Info
...
No Figma component set is mapped for `al-icon-info` in the **Altitude Design System** parity manifest (parity status: `missing-in-figma`). Map one in Figma, then run `pnpm run parity:seed`.
...
- Import: `@southleft/al-web-components/components/icon/icons/info.ts`
- Tag: `al-icon-info`
...
Source contract: `.altitude/contracts/altitude/al-icon-info.contract.json`.
