<!-- GENERATED by scripts/contracts/build-component-docs.mjs from al-icon-star.contract.json + the altitude parity manifest — do not hand-edit; run `pnpm contracts:docs` -->

# al-icon-star — Icon Star
...
No Figma component set is mapped for `al-icon-star` in the **Altitude Design System** parity manifest (parity status: `missing-in-figma`). Map one in Figma, then run `pnpm run parity:seed`.
...
- Import: `@southleft/al-web-components/components/icon/icons/star.ts`
- Tag: `al-icon-star`
...
Source contract: `.altitude/contracts/altitude/al-icon-star.contract.json`.
