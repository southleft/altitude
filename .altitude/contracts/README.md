...
node scripts/contracts/generate-figma.mjs --component al-button --sheet      # plugin-free documentation sheet, next to the set
```
...
resolution, enum-axis selection), `derive-sheet-plan.mjs`
(`buildSheetPlan()` + label humanizers + border rules), `conventions.mjs`
...
icon-wrapper name), `sheet-style.mjs` (pure presentation: separator purple,
dash pattern, cell pitch defaults, doc-header wiring),
`plugin-snippets.mjs` (the single copy of the plugin-side helpers both
emitters compose), `build-set-code.mjs` and `build-sheet-code.mjs` (the two
`figma_execute` code emitters — the set builder hosts the plugin-side
Phosphor/wrapper resolution and recoloring functions).
...
icon size variable, full-width margin, label typography, sheet cell pitch,
...
`scripts/contracts/figma/derive-ops.mjs`'s `buildOps()` — and `--sheet`
reuses exactly this machinery internally
(`buildOps(contract, { forceAllBooleanAxes: true })`) rather than
re-implementing the fan-out a second way.
...
## Documentation sheet (`--sheet`)

A plugin-free Propstar-equivalent documentation grid — no plugin install
required (an agent cannot launch a Figma plugin; a colleague may not have
Propstar). Propstar remains a valid OPTIONAL way to build the same kind of
sheet by hand; `--sheet` is the CANONICAL, automatable equivalent.

```bash
node scripts/contracts/generate-figma.mjs --component al-button              # 1. build/rebuild the lean set first
node scripts/contracts/generate-figma.mjs --component al-button --sheet      # 2. THEN build/replace its documentation sheet
node scripts/contracts/generate-figma.mjs --component al-button --sheet --ops-only          # sheet PLAN artifact only
node scripts/contracts/generate-figma.mjs --component al-button --sheet --check-determinism # plan derived twice, byte-compared
```

**What it builds.** A frame named `"<Component> — Prop Sheet"` — its OWN
top-level frame on `--page`, next to (never inside) the set's
`"<Component> — Generated"` presentation frame — a "Documentation Header"
instance on top, then a nested-auto-layout TABLE of real INSTANCES of the
already-built (property-mode) set, one per State × Variant × every other
BOOLEAN property combination (al-button: 5 × 5 × 2 × 2 = 100), each
switched via `setProperties` against the TARGET set's real property
definitions — never a freshly-built component. Idempotent: re-running
REPLACES the prior sheet frame by name; the target set is a read-only
lookup, never touched.

**A real table, CSS `border-collapse`-simple.** Nested auto-layout frames
— one VERTICAL "Sheet Grid" holding a header row and one frame per Variant
group (banner row + one row per boolean combo), each row a HORIZONTAL
frame of FIXED-width cells — never a `createVector()` line, never manual
x/y. Exactly three stroke sources: the outer "Sheet Grid" frame draws a
full four-side border once; every ROW draws only its own bottom edge
(none on the absolute last row); every CELL only its own right edge (none
on the row's last cell). A Variant-group boundary is that group's last
row's bottom weight upgraded to `SHEET_SEPARATOR_GROUP_WEIGHT` (2px vs
1px). `itemSpacing: 0` everywhere makes the single-edge strokes read as
ONE continuous line — exact pure-function rule: `rowBottomWeight()`/
`cellRightWeight()` in `scripts/contracts/figma/derive-sheet-plan.mjs`.
Cells get real bindable padding (`SHEET_CELL_PADDING_FIGMA_VAR`,
`theme/space/sm`); the grid its own (`SHEET_GRID_PADDING_FIGMA_VAR`,
`theme/space/lg`). (Corrected mid-T32 after owner review —
`DECISIONS.md` § T32.)

**Grouping (a documented judgment call — `buildSheetPlan()`'s comment in
`derive-sheet-plan.mjs`).** COLUMNS = State. ROW GROUPS, outermost to
innermost = Variant, then every other boolean axis in
`BOOLEAN_AXIS_CANONICAL_ORDER` (`conventions.mjs`: Slot Before, Slot
After, Is Full Width). "A clean deterministic grouping," not a
pixel-for-pixel replica of any reference screenshot.

**Labels are humanized, never a raw property dump.** Headers/banners read
the bare axis VALUE (`"Hover"`, never `"State=Hover"`); a row label
describes only what is ON (`"Default"`, `"Icon before"`,
`"Icons before + after"`, `"Content after"` for a slot with no
`figmaPlaceholder`, `"With full width"` for a non-slot boolean). Pure
string derivation — `humanizeAxisValue()`/`humanizeBooleanCombo()`/
`slotNounFor()` in `derive-sheet-plan.mjs`, covered by
`--check-determinism`.

**Documentation-page header.** An INSTANCE of the file's "Documentation
Header" component (a single COMPONENT, no exposed properties — text
overridden by editing child TEXT nodes directly), resolved BY NAME on the
file's "Documentation" page, never by node id. Contract-derived content:
"Heading" = `contract.name`; "Sub Heading" left UNTOUCHED ("Altitude
Design System"); "Description" = the contract's `description`
(trimmed/capped) plus a trailing HYPERLINKED "View full documentation"
run. **The link is a DUMMY placeholder** —
`https://altitude.pages.dev/docs/components/<tag-minus-al->/`, the docs
site's future per-component routing shape; revisit once docs are published
per-component. See `SHEET_DOC_HEADER_MASTER_NAME` and neighbors in
`scripts/contracts/figma/sheet-style.mjs`.

**Batched across multiple `figma_execute` calls, by design** (the ~30s
ceiling — Phosphor pitfall 1): one SETUP call (sheet frame, doc header,
table header row), then one call PER Variant row group (al-button: 5
calls, 20 instances each). Every cell uses a FIXED width
(`SHEET_ROW_LABEL_WIDTH_PX`/`SHEET_CELL_WIDTH_PX` in `sheet-style.mjs`;
per-component override via `figma.gen.json`) rather than measuring
rendered size — measure-after-build is exactly the per-call work a
~100-instance batch cannot afford under the ceiling.

**Requires the set to already exist.** `--sheet` never creates the lean
set or its page — run without `--sheet` first (or point at a real,
hand-built set of the same name on `--page`). Regenerating the base set
clears the WHOLE page, including a previously-built sheet frame — re-run
`--sheet` after any base regeneration.

**Ops artifact.** `.altitude/figma-sync/<project's figma-sync dir>/
generated-ops/<tag>.sheet.ops.json` — same gitignored zone, same
"deterministic build INPUT, no ids" rules (`buildSheetPlan()` is a pure
function; `--check-determinism` proves it the same way).
