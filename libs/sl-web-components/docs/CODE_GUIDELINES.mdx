...
- **Composition over inheritance** - SL Web Components adheres to the [composition over inheritance](https://en.wikipedia.org/wiki/Composition_over_inheritance) principle in order to create clean, extensible components that aren't tied to specific contexts or content.

# HTML <a name="html"></a>

## HTML principles and conventions <a name="html-principles"></a>

- **Use semantic markup.** This means marking up a document with information about the data structure behind the information. For example, use `<h1>` for headings, `<p>` for paragraphs, `<ol>` for ordered lists, and so on. Avoid using `<div>` and `<span>` for everything.
