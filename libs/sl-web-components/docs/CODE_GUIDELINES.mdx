...
Web Components is the web component implementation of SL, providing presentational UI components for web applications.

These coding principles and conventions are followed for HTML, CSS, and JavaScript:

- [Web Components principles](#principles)
- [HTML](#html)
...
## Web Components Principles <a name="principles"></a>

- **A Front-End Source of Truth**: Web Components serve as the UI-specific front-end code source of truth for SL, embodying best practices around accessibility, responsive design, performance, and other front-end practices. Applications can integrate Web Components to deliver high-quality web experiences.
- **Presentational UI Components Only**: Web Components provide a library of reusable [presentational UI components](https://medium.com/@dan_abramov/smart-and-dumb-components-7ca2f9a7c7d0) consumed by applications. These "dumb" components do not contain application business logic, data models, or rely on specific environments or configurations.
- **Agnostic**: Web Components are free of any framework, CMS, or implementation-specific conventions, aiming to function in any web-based environment. Framework, CMS, or implementation-specific handling should be managed within the application environment.
- **Predictable APIs**: Web Components offer consistent, clear [component APIs](#api-naming) to ensure a smooth developer experience.
- **Composition over Inheritance**: Web Components adhere to the [composition over inheritance](https://en.wikipedia.org/wiki/Composition_over_inheritance) principle, creating clean, extensible components not tied to specific contexts or content.
...
- `size` should be used for adjusting size attributes (e.g. `size="md"` or `size="xl"`). Default to `sm`, `lg`, `xl` with "md" being the undeclared default. Note: use abbreviated t-shirt sizes — `xs`, `sm`, `md`, `lg`, `xl` — for naming versus spelling out "small", "large" and so on. "Medium" (`md`) is the default value and should be omitted from the available options.
...
Source SVG icons live in the `icons` directory. A `build:icons` script is used to create the directories of icon components in both Web Components and React. These directories contain all individual SVG icon components to be used by the library.
...
### Folder Structure <a name="directory"></a>
