...
export class SLTabs extends SLElement {
   ...
   */
  @query('.sl-c-tabs__list')
  accessor tabsList: HTMLElement;

  /**
   * Query all the SLTab's
   */
  @queryAssignedElements({ flatten: true, slot: '' })
  accessor tabItems: Array<SLTab>;

  /**
   * Query all the SLTabPanel's
   */
  @queryAssignedElements({ flatten: true, slot: 'panel' })
  accessor tabPanels: Array<SLTabPanel>;
  ...
  syncTabWithPanel() {
    ...
    this.tabItems.forEach((tab, index) => {
      /* 2 */
      const tabAriaId = 'sl-c-tab__' + nanoid();
      /* 3 */
      tab.idx = index;
      tab.ariaId = tabAriaId;
      /* 4 */
      const tabPanelAriaId = `sl-c-tab-panel__` + nanoid();
  ...
  /**
   * Set the 'isScrollable' state based on whether the .sl-c-tabs__list element is overflowing.
  ...
  handleOnTabSelect(e: CustomEvent<{ target: Event }>) {
    ...
    /* 2 */
    this.activeTab = e.target as SLTab;
  ...
  handleOnTabKeydown(e: KeyboardEvent) {
    ...
    if (focused.matches('sl-tab')) {
      switch (e.key) {
        case 'ArrowRight':
          e.preventDefault();
          this.setActiveAdjacentTab(target as SLTab, false, true, null);
          break;
        case 'ArrowLeft':
          e.preventDefault();
          this.setActiveAdjacentTab(target as SLTab, true, true, null);
          break;
        case 'Home':
          e.preventDefault();
   ...
   * 1. Query the shadow DOM of the active tab for the button element with the class 'sl-c-tab'
   * 2. Set the focus on the found button element, making it the active element
   */
  setTabFocus() {
    const buttonElement = this.activeTab.shadowRoot.querySelector<HTMLButtonElement>('.sl-c-tab'); /* 1 */
   ...
   */
  async setActiveAdjacentTab(currentTab: SLTab, isPrevious: boolean, tabFocusOnly: boolean, e: Event) {
  ...
  render() {
    const componentClassNames = this.componentClassNames('sl-c-tabs', {
      'sl-is-scrollable': this.isScrollable,
      'sl-c-tabs--stretch': this.variant === 'stretch'
    });

    return html`
      <div class="${componentClassNames}">
        <header class="sl-c-tabs__header">
          ${this.isScrollable
            ? html`
                <${this.buttonEl}
                  class="sl-c-tabs__arrow sl-c-tabs__arrow--prev"
                  variant="tertiary"
                  ?hideText=${true}
          ...
          ${this.isScrollable
            ? html`
                <${this.buttonEl}
                  class="sl-c-tabs__arrow sl-c-tabs__arrow--next"
                  variant="tertiary"
                  ?hideText=${true}
