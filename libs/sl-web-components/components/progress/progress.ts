 ...
 * - The progress is a progress indicator that visually shows a remaining value.
 */
export class SLProgress extends SLElement {
  ...
  render() {
    const componentClassNames = this.componentClassNames('sl-c-progress', {
      'sl-is-circle-md': this.circleSize === 'md',
      'sl-is-circle-lg': this.circleSize === 'lg',
      'sl-is-circle-xl': this.circleSize === 'xl',
      'sl-has-label': this.showLabel === true
    });
    ...
    return html`
      <div class="${componentClassNames}" role="progressbar" aria-label=${this.labelAria} aria-valuenow=${this.currentProgress}>
        ${this.isCircle
          ? html`
              <div class="sl-c-progress__track--circle"></div>
              <svg class="sl-c-progress__fill--circle" xmlns="http://www.w3.org/2000/svg">
                <circle
                  cx="8"
                  cy="8"
                  r="8"
            ...
            `
          : html`
              <div class="sl-c-progress__track">
                <div class="sl-c-progress__fill" style="width: ${this.currentPercentage}%"></div>
              </div>
            `}
        <div class="sl-c-progress__label${this.showLabel ? ' ' : ' sl-u-is-vishidden'}">${this._label}</div>
      </div>
    `;
  }
}
