 ...
 *
 * Radio group allows users to choose one option from a set of mutually exclusive options.
 * - **slot**: The radio content, a set of radio items
 * - **slot** "field-note": If content is slotted, it will display in place of the fieldNote property
 * - **slot** "error": If content is slotted, it will display in place of the errorNote property
 */
export class SLRadioGroup extends SLElement {
  static el = 'sl-radio-group';
   ...
   */
  @state()
  accessor checkedItem: SLRadio;

  /**
   * Query all the radio's
   */
  @queryAssignedElements({ flatten: true, slot: '' })
  private accessor radioItems: Array<SLRadio>;
  ...
  constructor() {
     ...
     */
    this.addEventListener('onRadioChange', (e: CustomEvent) => this.handleOnRadioChange(e.target as SLRadio));
  }
   ...
   */
  handleOnRadioChange(item: SLRadio) {
    if (this.checkedItem) {
   ...
   */
  setCheckedAdjacentItem(checkedItem: SLRadio, isPrevious: boolean) {
    ...
    if (this.checkedItem) {
      this.checkedItem.shadowRoot?.querySelector<HTMLInputElement>('.sl-c-radio__input:not(:disabled)').focus();
    }
    /* 9 */
    this.dispatch({
  ...
  handleOnKeydown(e: KeyboardEvent) {
    ...
    if (e.code === 'Enter') {
      this.handleOnRadioChange(target as SLRadio); /* 2 */
    } else if (e.code === 'ArrowLeft' || e.code === 'ArrowUp') {
      this.setCheckedAdjacentItem(target as SLRadio, true); /* 3 */
    } else if (e.code === 'ArrowRight' || e.code === 'ArrowDown') {
      this.setCheckedAdjacentItem(target as SLRadio, false); /* 4 */
    }
  }

  render() {
    const componentClassNames = this.componentClassNames('sl-c-radio-group', {
      'sl-is-error': this.isError === true,
      'sl-is-disabled': this.isDisabled === true,
      'sl-has-hidden-legend': this.hideLegend,
      'sl-c-radio-group--horizontal': this.variant === 'horizontal'
    });

    return html`
      <fieldset class="${componentClassNames}" @keydown=${this.handleOnKeydown}>
        ${this.label && html` <legend class="sl-c-radio-group__legend" aria-describedby="${this.ariaDescribedBy}">${this.label}</legend> `}
        <div class="sl-c-radio-group__list">
          <slot></slot>
        </div>
...
if ((globalThis as any).slAutoRegistry === true && customElements.get(SLRadioGroup.el) === undefined) {
  customElements.define(SLRadioGroup.el, SLRadioGroup);
}
