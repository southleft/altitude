...
export class SLCalendar extends SLElement {
   ...
   */
  @query('.sl-c-calendar__month-selector-popup')
  accessor calendarMonthPopup: HTMLElement;

  /**
   * Query the month selector button
   */
  @query('.sl-c-calendar__month-selector-button')
  ...
  updated() {
    setTimeout(() => {
      const currentMonth: HTMLElement = this.calendarMonthPopup.querySelector('.sl-is-current');
      const activeMonth: HTMLElement = this.calendarMonthPopup.querySelector('.sl-is-active');
      if (activeMonth) {
        /* 1 */
  ...
  updateGrid(date: Date) {
    if (date !== undefined) {
      ...
      /* 1 */
      const monthButton = this.shadowRoot.querySelector<HTMLButtonElement>('.sl-c-calendar__month-selector-button');
      if (monthButton) {
        monthButton.focus();
      }
    }
  ...
  toggleMonthPopup() {
    ...
    setTimeout(() => {
      ...
      this.shadowRoot.querySelector('#sl-year-' + monthToggleYear).scrollIntoView({ block: 'nearest' });
    }, 1);
  }
  ...
  render() {
    const componentClassNames = this.componentClassNames('sl-c-calendar', {});

    return html`
      <div class="${componentClassNames}">
        <nav class="sl-c-calendar__header">
          <${this.buttonEl}
            type="button"
            variant="tertiary"
            class="sl-c-calendar__nav-btn"
        ...
        <div class="sl-c-calendar__table-container">
          <table role="presentation" class="sl-c-calendar__table">
            <thead class="sl-c-calendar__table-header">
              <tr class="sl-c-calendar__table-row">
                ${this.setWeekdaysHeader().map((day: any) => {
                  return html` <th abbr="${day}" class="sl-c-calendar__header-cell">${day}</th> `;
                })}
              </tr>
            </thead>
            <tbody class="sl-c-calendar__table-body">
              ${this.days.map((week: any) => {
                return html`
                  <tr class="sl-c-calendar__table-row">
                    ${week.map((day: any) => {
                      return html`
                        <td class="sl-c-calendar__table-cell">
                          ${day.value !== null
                            ? html`
                                <button
                                  type="button"
                                  class="sl-c-calendar__item
                                  ${this.setActive(day) ? 'sl-is-active' : ''}
                                  ${this.setAvailableDay(day.value) ? 'sl-is-available' : ''}
                                  ${this.setToday(day) ? 'sl-is-today' : ''}"
        ...
        <div class="sl-c-calendar__month-selector-popup" role="dialog" ?hidden="${!this.showMonthPopup}" @keydown=${this.handleOnKeydown}>
          <div class="sl-c-month-selector">
            <ul class="sl-c-month-selector__list">
              ${this.years.map((year: any) => {
                return html`
                  <li class="sl-c-month-selector__item" id="sl-year-${format(year.value, 'y')}">
                    <h4 class="sl-c-month-selector__year-heading">${format(year.value, 'y')}</h4>
                    <ul class="sl-c-month-selector__sub-list">
                      ${year.month.map((month: any) => {
                        return html`
                          <li class="sl-c-month-selector__sub-item">
                            <button
                              class="sl-c-month-selector__button
                              ${isSameMonth(month, this.currentDate) ? 'sl-is-current' : ''}
                              ${isEqual(startOfMonth(startOfDay(this.navDate)), startOfMonth(startOfDay(month))) ? 'sl-is-active' : ''}"
