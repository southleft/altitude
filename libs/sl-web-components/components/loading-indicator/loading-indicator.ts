...
export class SLLoadingIndicator extends SLElement {
  ...
  render() {
    const componentClassNames = this.componentClassNames('sl-c-loading-indicator', {
      'sl-c-loading-indicator--small': this.small === true,
      'sl-c-loading-indicator--inverted': this.inverted === true
    });
...
if ((globalThis as any).enAutoRegistry === true && customElements.get(SLLoadingIndicator.el) === undefined) {
  customElements.define(SLLoadingIndicator.el, SLLoadingIndicator);
}

declare global {
  interface HTMLElementTagNameMap {
    'sl-loading-indicator': SLLoadingIndicator;
  }
}
