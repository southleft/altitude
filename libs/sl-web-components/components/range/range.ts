 ...
 */
export class SLRange extends SLElement {
  static el = 'sl-range';
  ...
  checkEquality(rangeType: string) {
    ...
    if (lowerVal === upperVal) {
      if (rangeType === UPPER_RANGE && !this.upperRange.classList.contains('sl-c-range__input-upper-active-thumb')) {
        const percentage = this.calculatePercentage(this.upperRangeValue);
        this.upperRange.classList.add('sl-c-range__input-upper-active-thumb');
        // @ts-ignore
        this.upperRangeLabelContainer.style.insetInlineStart = `calc(${percentage + 0.3}% + (${10 - percentage * 0.2}px))`;
      } else if (rangeType === LOWER_RANGE && !this.lowerRange.classList.contains('sl-c-range__input-lower-active-thumb')) {
        const percentage = this.calculatePercentage(this.lowerRangeValue);
        this.lowerRange.classList.add('sl-c-range__input-lower-active-thumb');
        // @ts-ignore
        this.lowerRangeLabelContainer.style.insetInlineStart = `calc(${percentage - 0.3}% + (${10 - percentage * 0.2}px))`; /* 6 */
      }
    } else if (lowerVal !== upperVal) {
      if (rangeType === UPPER_RANGE && this.upperRange.classList.contains('sl-c-range__input-upper-active-thumb')) {
        this.upperRange.classList.remove('sl-c-range__input-upper-active-thumb');
      } else if (rangeType === LOWER_RANGE && this.lowerRange.classList.contains('sl-c-range__input-lower-active-thumb')) {
        this.lowerRange.classList.remove('sl-c-range__input-lower-active-thumb');
      }
    }
  }
  ...
  renderRangeRange() {
    return html`
      ${this.label || this.slotNotEmpty('label') ? html`<slot name="label"><label class="sl-c-range__label">${this.label}</label></slot>` : html``}
      <div class="sl-c-range__input-container">
        <slot name="before"> <span class="sl-c-range-prefix-text">${this.min}</span></slot>
        <div class="sl-c-range__input">
          <input
            class="sl-c-range__input-one sl-c-range__input-input"
            type="range"
          ...
          />
          <span class="sl-c-range__input-color"></span>
          <input
            class="sl-c-range__input-two sl-c-range__input-input"
            type="range"
            min=${ifDefined(this.min)}
            max=${ifDefined(this.max)}
          ...
          />
          <div>
            <span class="sl-c-range__input-one-label-container"><span class="sl-c-range__input-one-label"> ${this.lowerRangeValue} </span></span>
            <span class="sl-c-range__input-two-label-container"><span class="sl-c-range__input-two-label"> ${this.upperRangeValue}</span></span>
          </div>
        </div>
        <slot name="after"> <span class="sl-c-range-suffix-text">${this.max}</span></slot>
        ${this.hasOutput
          ? html`
              <div>
                <input
                  ...
                  class="sl-c-range__output-range-one"
                  @input=${this.syncOutFieldToLowerRange}
                  ?disabled="${this.isDisabled}"
                />
                <div class="sl-c-range__output-range-one-text">Min</div>
              </div>
            `
          : ''}
        ${this.hasOutput
          ? html`
              <div>
                <input
                  ...
                  class="sl-c-range__output-range-two"
                  @input=${this.syncOutFieldToUpperRange}
                  ?disabled="${this.isDisabled}"
                />
                <div class="sl-c-range__output-range-two-text">Max</div>
              </div>
            `
          : ''}
      </div>
  ...
  render() {
    const componentClassNames = this.componentClassNames('sl-c-range', {
      'sl-c-range--range': this.behavior === 'range',
        ...
        ${this.behavior === 'range'
          ...
          : html`
              ${this.label || this.slotNotEmpty('label')
                ? html`<slot name="label"><label class="sl-c-range__label">${this.label}</label></slot>`
                : html``}
              <div class="sl-c-range__output-container">
                <slot name="before"><span class="sl-c-range-prefix-text">${this.min}</span></slot>
                <div class="sl-c-range__tooltip-container">
                  ${this.hasTooltip
                    ? html` <span class="sl-c-range__tooltip-wrapper"><output class="sl-c-range__tooltip"> ${this.result} </output></span> `
                    : ''}
                  <input
                    ?disabled="${this.isDisabled}"
                    class="sl-c-range__input"
                    type="range"
                    min=${ifDefined(this.min)}
                    max=${ifDefined(this.max)}
                    step=${ifDefined(this.step)}
