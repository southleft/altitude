...
/**
 * SCSS Token Variable Formatting
 ...
 * 9. Wrap the variables inside :root{}
 */
StyleDictionary.registerFormat({
  name: 'scss/variables-tokens',
  formatter: function ({ dictionary, options }) {
    /* 1 */
    const otherVariables = dictionary.allTokens
      .filter(token => !token.name.endsWith('-italic') && !token.name.endsWith('-underline') && !token.name.includes('breakpoint'))
      .map(token => {
        let name;
        /* 2 */
...
StyleDictionary.registerFormat({
  ...
  formatter: function ({ dictionary, options }) {
    ...
    const themeVariables = (theme) => {
      return `$${theme}-theme-prefix: '${theme}';\n`;
    };
    const themeDeclaration = themeVariables(theme);
    /* 6 */
