...
export class ALInput extends ALElement {
  ...
  firstUpdated() {
    ...
    /* 3 */
    if (this.isFocused) {
      this.shadowRoot.querySelector<HTMLTextAreaElement>('.al-c-input__input').focus();
    }
  }
  ...
  setBeforePadding(beforeEl: HTMLElement) {
    ...
    this.style.setProperty('--al-input-padding-start', beforeWidth + 'px');
  }
  ...
  async setInputPadding() {
    ...
    /* 2 */
    const afterEl = this.shadowRoot.querySelector<HTMLElement>('.al-c-input__after');
    if (afterEl) {
      const afterWidth = afterEl.clientWidth + 24;
      this.style.setProperty('--al-input-padding-end', afterWidth + 'px');
    }
  }
    ...
    return html`
      <div class="${componentClassNames}">
            ...
            ? html`
                <div class="al-c-input__before">
                  ...
                  ${this.isRequired && this.hideLabel
                    ? html` <span class="al-c-input__asterisk al-c-input__asterisk--hidden-label">*</span> `
                    : html``}
                </div>
              `
            : html``}
        ...
        ${this.maxLength || this.fieldNote || this.errorNote || this.slotNotEmpty('field-note') || this.slotNotEmpty('error')
          ? html`
              <div class="al-c-input__footer">
                <div class="al-c-input__field-notes">
...
if ((globalThis as any).alAutoRegistry === true && customElements.get(ALInput.el) === undefined) {
  customElements.define(ALInput.el, ALInput);
}
