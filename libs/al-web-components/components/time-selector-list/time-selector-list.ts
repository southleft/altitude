  ...
  render() {
    ...
    return html`
      <div class="${componentClassNames}">
        <div class="al-c-time-selector-list__inner">
          <h4 class="al-c-time-selector-list__heading">${this.timeSelectorLabel}</h4>
          <ul class="al-c-time-selector-list__list">
            ${this.times.map((time: { value: Date; available: boolean }) => {
              return html`
                <li class="al-c-time-selector-list__item" id="al-time-${format(time.value, 'hmma')}">
                  ${time.value.getTime() !== 0
                    ? html`
                        <button
                          class="al-c-time-selector-list__button
                          ${this.setActive(time) ? 'al-is-active' : ''}
                          ${this.setAvailable(time) ? 'al-is-available' : ''}"
...
if ((globalThis as any).alAutoRegistry === true && customElements.get(ALTimeSelectorList.el) === undefined) {
  customElements.define(ALTimeSelectorList.el, ALTimeSelectorList);
}

declare global {
  interface HTMLElementTagNameMap {
    'al-time-selector-list': ALTimeSelectorList;
  }
}
