...
import { ALIconSearch } from '../icon/icons/search';
...
export class ALSearch extends ALElement {
  ...
  private iconSearchEl = unsafeStatic(this.elementMap.get(ALIconSearch.el));
   ...
   */
  @query('.al-c-search--dynamic .al-c-search__dropdown-panel')
  ...
  accessor currentSelectedItem: ALListItem;
   ...
   */
  setLists() {
    if (this.children[0]?.outerHTML.includes(this.elementMap.get(ALList.el))) {
      const thisList = this.querySelector(this.elementMap.get(ALList.el));

      setTimeout(() => {
        thisList?.setAttribute('role', 'listbox');
        thisList?.setAttribute('aria-controls', this.ariaControlsId);
        this.listItems.forEach((item: ALListItem) => {
  ...
  clearSearch() {
    this.isActiveDropdown = false;
    this.value = '';
    this.shadowRoot.querySelector<HTMLInputElement>('.al-c-search__input').value = ''; /* 2 */
    this.isError = false;
    this.shadowRoot.querySelector<HTMLInputElement>('.al-c-search__input').focus(); /* 3 */
  ...
  toggleActive(e: Event) {
    ...
    if ((e.target as ALListItem).value) {
      this.shadowRoot.querySelector<HTMLInputElement>('.al-c-search__input').value = (e.target as ALListItem).value as string;
      this.isActiveDropdown = true;
    }

    setTimeout(() => {
      const elem = this.shadowRoot.querySelector<HTMLInputElement>('.al-c-search__input');
   ...
   */
  updateScroll(selectedItem: ALListItem) {
    const dropdownPanelBody = this?.shadowRoot
      .querySelector(this.elementMap.get(ALDropdownPanel.el))
      ?.shadowRoot.querySelector('.al-c-dropdown-panel__body');
  ...
  handleKeyDown(e: KeyboardEvent) {
    ...
    if (
      ...
      let currentItem: ALListItem = this.querySelector(this.elementMap.get(ALList.el))?.querySelector<ALListItem>(this.elementMap.get(ALListItem.el));

      while (currentItem.isDisabled) {
        currentItem = currentItem.nextElementSibling as ALListItem;
      }
      ...
      if (e.code === 'Tab') {
        ...
        setTimeout(() => {
          this.closePanel();
          this.shadowRoot
            .querySelector<HTMLInputElement>('.al-c-search__clear-button')
            .shadowRoot.querySelector<HTMLButtonElement>('.al-c-button')
            .focus();
        }, 1);
      }
      if (e.code === 'Enter') {
        /* 5 */
        this.shadowRoot.querySelector<HTMLInputElement>('.al-c-search__input').value = this.currentSelectedItem.value as string;
        setTimeout(() => {
          this.closePanel();
        }, 1);
      ...
      if (this.currentSelectedItem?.previousElementSibling && e.code === 'ArrowUp') {
        /* 12 */
        let currentItem = this.currentSelectedItem?.previousElementSibling as ALListItem;

        while (currentItem?.isDisabled) {
          currentItem = currentItem.previousElementSibling as ALListItem;
        }
        if (currentItem === null) {
        ...
        this.currentSelectedItem.removeAttribute('aria-selected');
        this.currentSelectedItem = <ALListItem>currentItem;
        this.currentSelectedItem.setAttribute('aria-selected', 'true');
        this.ariaActiveDescendantId = this.currentSelectedItem.getAttribute('id');
      } else if (this.currentSelectedItem?.nextElementSibling && e.code === 'ArrowDown') {
        /* 13 */
        let currentItem = this.currentSelectedItem?.nextElementSibling as ALListItem;
  ...
  dynamicPosition() {
    if (this._enDropdownPanel) {
      ...
      const listHeight = this.querySelector(this.elementMap.get(ALList.el))?.shadowRoot?.querySelector('.al-c-list').clientHeight; /* 2 */
      const inputBottom = this.shadowRoot.querySelector('.al-c-search__input').getBoundingClientRect().bottom; /* 2 */
  ...
  render() {
    const componentClassName = this.componentClassNames('al-c-search', {
      'al-c-search--with-button': this.buttonText,
      'al-c-search--top': this.position === 'top',
      'al-c-search--dynamic': this.isDynamic === true,
      ...
      'al-is-focused-in': this.isFocusedIn === true,
      'al-has-hidden-label': this.hideLabel
    });

    const itemTemplates: ALListItem[] = [];
    this.listItems.forEach((item) => {
      item.setAttribute('value', <string>item.value);
      const newItem = unsafeHTML(item.outerHTML);
      itemTemplates.push(<ALListItem>newItem);
    });

    return html`
      <div class="${componentClassName}">
        <div class="al-c-search__container">
          <${this.inputEl}
            class="al-c-search__input"
            type="text"
            id="${this.fieldId}"
            label="${this.label}"
          ...
          ${
            this.isEmpty && this.slotNotEmpty('emptyMessage')
              ? html`
                <${this.dropdownPanelEl} class="al-c-search__dropdown-panel" @keydown=${this.handleKeyDown} ?hasScroll=${true}>
                  ${
                    this.slotNotEmpty('emptyMessage') &&
                    html`
                      <div class="al-c-search__empty-message">
        ...
        ${
          this.buttonText
            ? html`
              <${this.buttonEl} class="al-c-search__submit-button" text=${this.buttonText} variant="primary" ?disabled="${this.isDisabled}">
                ${this.buttonText}
              </${this.buttonEl}>
            `
            : ''
...
if ((globalThis as any).alAutoRegistry === true && customElements.get(ALSearch.el) === undefined) {
  customElements.define(ALSearch.el, ALSearch);
}
