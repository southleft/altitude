...
export class ALListItem extends ALElement {
  ...
  focusOnFirstItem(e: Event) {
    const listItems = this.parentNode.querySelectorAll(this.elementMap.get(ALListItem.el));
    for (let i = 0; i < listItems.length; i++) {
      if (!listItems[i].shadowRoot.querySelector('.al-c-list-item.al-is-disabled')) {
        listItems[i].shadowRoot.querySelector<HTMLButtonElement | HTMLAnchorElement>('.al-c-list-item__link').focus();
        e.preventDefault();
        break;
      }
    }
  ...
  handleKeyDown(e: KeyboardEvent) {
    if (this.previousElementSibling && (e.code === 'ArrowLeft' || e.code === 'ArrowUp')) {
      ...
      const previousElement = previous.shadowRoot?.querySelector<HTMLInputElement>('.al-c-list-item__link');
      if (previousElement) {
        previousElement.focus();
        e.preventDefault();
        while (previous) {
          const previousListItem = previous.shadowRoot.querySelector<HTMLLIElement>('.al-c-list-item');
          if (!previousListItem.classList.contains('al-is-disabled')) {
            const newPreviousElement = previous.shadowRoot?.querySelector<HTMLInputElement>('.al-c-list-item__link');
      ...
      const nextElement = next.shadowRoot?.querySelector<HTMLInputElement>('.al-c-list-item__link');
      if (nextElement) {
        while (next) {
          const nextListItem = next.shadowRoot.querySelector<HTMLLIElement>('.al-c-list-item');
          if (!nextListItem.classList.contains('al-is-disabled')) {
            const newNextElement = next.shadowRoot?.querySelector<HTMLInputElement>('.al-c-list-item__link');
  ...
  render() {
    const componentClassName = this.componentClassNames('al-c-list-item', {
      ...
      'al-c-list-item--flyout-position-left': this.flyoutPosition === 'left',
      'al-c-list-item--flyout-position-bottom': this.flyoutPosition === 'bottom',
      'al-c-list-item--flyout-position-top': this.flyoutPosition === 'top',
...
if ((globalThis as any).alAutoRegistry === true && customElements.get(ALListItem.el) === undefined) {
  customElements.define(ALListItem.el, ALListItem);
}

declare global {
  interface HTMLElementTagNameMap {
    'al-list-item': ALListItem;
  }
}
