...
export class ALTextarea extends ALElement {
  ...
  firstUpdated() {
    ...
    /* 4 */
    if (this.isFocused) {
      this.shadowRoot.querySelector<HTMLTextAreaElement>('.al-c-textarea__input').focus();
    }
  }
  ...
  setInputPadding() {
    /* 1 */
    const beforeEl = this.shadowRoot.querySelector<HTMLElement>('.al-c-textarea__before');
    if (beforeEl) {
      let beforeWidth;
      ...
      this.style.setProperty('--al-textarea-padding-start', beforeWidth + 'px');
    }
    /* 2 */
    const afterEl = this.shadowRoot.querySelector<HTMLElement>('.al-c-textarea__after');
    if (afterEl) {
      const afterWidth = afterEl.clientWidth + 24;
      this.style.setProperty('--al-textarea-padding-end', afterWidth + 'px');
    }
  }

  render() {
    const componentClassNames = this.componentClassNames('al-c-textarea', {
      ...
      <div class="${componentClassNames}">
        <div class="al-c-textarea__container">
            ...
            ? html`
                <div class="al-c-textarea__before">
                  ...
                  ${this.isRequired && this.hideLabel
                    ? html` <span class="al-c-textarea__asterisk al-c-textarea__asterisk--hidden-label">*</span> `
                    : html``}
                </div>
              `
            : html``}
          ${this.slotNotEmpty('after')
            ? html`
                <div class="al-c-textarea__after">
        ...
        ${this.maxLength || this.fieldNote || this.errorNote || this.slotNotEmpty('field-note') || this.slotNotEmpty('error')
          ? html`
              <div class="al-c-textarea__footer">
                <div class="al-c-textarea__field-notes">
...
if ((globalThis as any).alAutoRegistry === true && customElements.get(ALTextarea.el) === undefined) {
  customElements.define(ALTextarea.el, ALTextarea);
}

declare global {
  interface HTMLElementTagNameMap {
    'al-textarea': ALTextarea;
  }
}
