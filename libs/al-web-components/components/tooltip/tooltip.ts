...
export class ALTooltip extends ALElement {
  ...
  render() {
    const componentClassNames = this.componentClassNames('al-c-tooltip', {
      'al-c-tooltip--top': this.position === 'top',
      'al-c-tooltip--bottom': this.position === 'bottom',
      'al-c-tooltip--left': this.position === 'left',
      'al-c-tooltip--right': this.position === 'right',
      'al-is-dynamic': this.isDynamic === true,
      'al-is-active': this.isActive === true,
      'al-is-interactive': this.isInteractive === true,
      'al-has-prefix': this.slotNotEmpty('prefix'),
      'al-has-arrow': this.hasArrow
    });
    ...
    return html`
      <div class="${componentClassNames}" @keydown=${this.handleOnKeydown}>
        ...
        `}
        <div class="al-c-tooltip__container" tabindex="-1" role="tooltip" id=${this.ariaDescribedBy} aria-hidden=${this.isActive ? false : true} style=${this.isActive ? "" : "display: none"}>
          ${this.slotNotEmpty('prefix') &&
          html`
            <div class="al-c-tooltip__prefix">
...
if ((globalThis as any).alAutoRegistry === true && customElements.get(ALTooltip.el) === undefined) {
  customElements.define(ALTooltip.el, ALTooltip);
}

declare global {
  interface HTMLElementTagNameMap {
    'al-tooltip': ALTooltip;
  }
}
