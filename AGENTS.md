...
| **medium** | **`.altitude/migration.json` entry** — net-new components start in state `scoped-complete` (no `dual` / `legacy` transition needed) with `react19: true, headless: …, ssr: …` reflecting the actual surface. |
